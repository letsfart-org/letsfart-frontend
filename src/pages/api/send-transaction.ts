...
import { fetchMintAccount } from "@/lib/dbUtils";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const { signedTransaction, mint } = req.body as SendTransactionRequest;
    ...
    if (!mintAccount) {
      return res.status(400).json({ error: 'mint account not found!' });
    }
    console.log("mintAccount: ", mintAccount);
    const keyPair = Keypair.fromSecretKey(bs58.decode(mintAccount.pkey));
