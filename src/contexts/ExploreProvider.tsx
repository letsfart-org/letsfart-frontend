...
const InterestingLaunchpads = ["letsbonk.fun","pump.fun","bags.fun","jup-studio","Believe","moonshot","moonit","trends.fun","met-dbc"];
const ExploreProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  console.log("POOL_CONFIG_KEY: ", process.env.POOL_CONFIG_KEY);
  console.log("NEXT_PUBLIC_POOL_CONFIG_KEY: ", process.env.NEXT_PUBLIC_POOL_CONFIG_KEY);
    ...
    return Object.fromEntries(
      Object.values(ExploreTab).map((tab) => [
        tab,
        {
          ...
          filters: {
            ...filtersConfig?.[tab],
            launchpads: InterestingLaunchpads,
          },
        },
      ])
    ) as Required<GemsTokenListQueryArgs>;
  }, [filtersConfig, InterestingLaunchpads]);
